...
drawstatusbar(Monitor *m, int bh, char *stext) {
	...
	/* compute width of status text (skip color codes) */
	w = 0;
	i = -1;
	while (text[++i]) {
		if (text[i] == '^') {
	...
	/* render status text, interpreting ^c^/^b^/^d^/^f^/^r^ codes */
	i = -1;
	isCode = 0;
	while (text[++i]) {
		if (text[i] == '^' && !isCode) {
...
movestack(const Arg *arg) {
	Client *c = NULL, *p = NULL, *pc = NULL, *i, *tmp;
	if (arg->i > 0) {
	...
	if (c && c != selmon->sel) {
		for (i = selmon->clients; i && i->next != selmon->sel; i = i->next); pc = i;
		for (i = selmon->clients; i && i->next != c;           i = i->next); p  = i;
		if (selmon->sel->next == c) {
			selmon->sel->next = c->next;
			c->next = selmon->sel;
			if (pc) pc->next = c; else selmon->clients = c;
		} else if (c->next == selmon->sel) {
			c->next = selmon->sel->next;
			selmon->sel->next = c;
			if (p) p->next = selmon->sel; else selmon->clients = selmon->sel;
		} else {
			tmp = selmon->sel->next;
			selmon->sel->next = c->next;
			c->next = tmp;
